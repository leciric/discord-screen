...
function socket() {
  return {
    ...
    // O dublê precisa ter close: derrubar uma transmissão abandonada fecha o
    // socket, e é o fechamento que faz a aba de captura soltar a tela — pedir
    // não basta, uma aba em segundo plano pode demorar a reagir.
    close() {
      this.fechado = true;
      this.readyState = 3;
    },
...
describe('sala vazia', () => {
  ...
  it('derruba a transmissão de quem saiu da atividade, e aí a sala fecha', () => {
    ...
    // Pedir é o caminho educado; fechar é o que garante. Sem o fechamento, uma
    // aba que não reage à mensagem continua com a tela capturada.
    expect(captura.fechado).toBe(true);
